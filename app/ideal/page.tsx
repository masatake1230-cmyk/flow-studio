  ...
  return (
    <main className="vstack">
        ...
        <div className="grid">
          {habits.map((h) => (
            <button
              ...
              style={{
                background: today.done[h.key] ? "#d1fae5" : "#fff",
                borderColor: today.done[h.key] ? "#10b981" : "#ddd",
              }}
            >
              {today.done[h.key] ? "✅ " : ""}
              {h.label}
      ...
      <section className="card vstack">
        ...
        <div className="row">
          <div className="card">
            <strong>達成チェック数</strong>
            <div style={{ fontSize: 24 }}>{week.checked}/{week.totalChecks}</div>
          </div>
          <div className="card">
            <strong>達成率</strong>
            <div style={{ fontSize: 24 }}>{week.rate.toFixed(0)}%</div>
          </div>
        </div>
        <small className="muted">
          ヒント: 達成率が60%を下回る週は「休息日」を明示的に入れると挽回しやすい。
        </small>
      </section>
